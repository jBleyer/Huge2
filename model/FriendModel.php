...
class FriendModel
{
    public static function befriendUser($user_id)
    {
        //echo $user_id;

        $database = DatabaseFactory::getFactory()->getConnection();
        $query = $database->prepare("UPDATE users SET is_friend = true WHERE user_id = :user_id LIMIT 1");
    ...
    public static function unfriendUser($user_id)
    {
        //echo $user_id;

        $database = DatabaseFactory::getFactory()->getConnection();
        $query = $database->prepare("UPDATE users SET is_friend = false WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(':user_id' => $user_id));
        if ($query->rowCount() == 1) {
            return true;
        }
